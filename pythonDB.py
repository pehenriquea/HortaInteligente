...
con = mysql.connector.connect(host='localhost', database='IoT', user='root', password='@dm!n!23')
...
def on_connect(client, userdata, flags, rc):
    ...
    client.subscribe("topico_publica_nivel_agua")
...
def on_message(client, userdata, msg):
    ...
    cursor = con.cursor()
    if msg.topic == "topico_publica_temp":
        cursor.execute("INSERT INTO daodsIoT (temp) VALUES ('{}')".format(str(msg.payload.decode())))
        con.commit()

        cursor.execute("SELECT temp FROM dadosIoT")
        myresult = cursor.fetchall()
        print(myresult)

    elif msg.topic == "topico_publica_nivel_agua":
        cursor.execute("INSERT INTO daodsIoT (dist) VALUES ('{}')".format(str(msg.payload.decode())))
        con.commit()

        cursor.execute("SELECT dist FROM dadosIoT")
        myresult = cursor.fetchall()
        print(myresult)
