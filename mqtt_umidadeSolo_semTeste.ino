...
//Declaração variáveis Sensor de Umidade do Solo
#define sensorUS 14
...
void reconnectMQTT(void)
{
  while (!MQTT.connected())
  {
    ...
    if (MQTT.connect(ID_MQTT))
    {
      ...
      MQTT.subscribe(TOPICO_SUBSCRIBE_LED);  //topicos inscritos para receber informações
    }
    else
    {
...
void umidadeSolo() {
  //MANTÉM valorLido DENTRO DO INTERVALO (ENTRE analogSoloMolhado E analogSoloSeco)
  valorLido = constrain(analogRead(sensorUS), analogSoloMolhado, analogSoloSeco);
...
void umidadeTemp(){
  ...
  /*converte a temperatura lida para string*/
